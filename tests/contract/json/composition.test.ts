...
parity("json + window", [
  ...
  `INSERT INTO t VALUES (1, '{"v":10}'), (2, '{"v":20}'), (3, '{"v":30}')`,
], `SELECT id, json_extract(data, '$.v') AS v,
          SUM(json_extract(data, '$.v')) OVER (ORDER BY id) AS running
   FROM t ORDER BY id`);

sequenceParity("json + view + transaction", [
  "CREATE TABLE t(data TEXT)",
  "CREATE VIEW v AS SELECT json_extract(data, '$.x') AS x FROM t",
], [
  { sql: "BEGIN" },
