...
matrixBoth("positional parameters in expressions", (memory, sqlite) => {
  expectParity(memory.query("SELECT ? + ? AS sum, ? AS text", [2, 3, "ok"]), sqlite.query("SELECT ? + ? AS sum, ? AS text", [2, 3, "ok"]));
});
matrixBoth("positional parameters in INSERT", (memory, sqlite) => {
  setupBoth(memory, sqlite, ["CREATE TABLE t(id INTEGER,name TEXT)"]);
  expectParity(memory.exec("INSERT INTO t VALUES (?,?)", [1, "Ada"]), sqlite.exec("INSERT INTO t VALUES (?,?)", [1, "Ada"]));
...
matrixBoth("named parameter slots bind by statement order", (memory, sqlite) => {
  expectParity(memory.query("SELECT :left + :right AS value", [7, 8]), sqlite.query("SELECT :left + :right AS value", [7, 8]));
});
