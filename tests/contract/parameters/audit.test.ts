...
parity("prepared statement binds NULL and BLOB", [
  "CREATE TABLE t(id INTEGER, b BLOB, n INT)",
], "SELECT typeof(?) a, typeof(?) b, hex(?) c", [null, new Uint8Array([1, 2]), new Uint8Array([1, 2])]);

sequenceParity("statement reuse after schema-compatible writes", [
...
matrixBoth("large integer parameter roundtrips within safe integer range", (memory, sqlite) => {
  ...
  expectParity(
    memory.query("SELECT v, typeof(v) kind FROM t"),
    sqlite.query("SELECT v, typeof(v) kind FROM t"),
  );
});
