...
parity(
  "ORDER BY applies NOCASE collation",
  textData,
  "SELECT value FROM words ORDER BY value COLLATE NOCASE, value",
);

parity(
  "equality applies explicit NOCASE collation",
  [],
  "SELECT 'a' = 'A' COLLATE NOCASE AS equal",
);

parity(
  "RTRIM ignores trailing spaces",
  [],
  "SELECT 'abc' = 'abc  ' COLLATE RTRIM AS equal",
);
...
sequenceParity(
  ...
  [
    ...
    "CREATE UNIQUE INDEX tags_value_nocase ON tags(value COLLATE NOCASE)",
  ],
  [
